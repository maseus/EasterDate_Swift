...
//Parametri dimensionali costanti di riferimento.
...
#define _UIKeyboardFrameEndUserInfoKey (&UIKeyboardFrameEndUserInfoKey != nil ? UIKeyboardFrameEndUserInfoKey : @"UIKeyboardBoundsUserInfoKey")
...
//Metodo gestore dell'apertura della tastiera.
- (void) keyboardViewAppear: (NSString *) animationID context: (void *) context
{
    self.keyboardAvoidingState.keyboardAnimationInProgress = YES;
}

//Metodo gestore della chiusura della tastiera.
- (void) keyboardViewDisappear: (NSString *) animationID finished: (BOOL) finished context: (void *) context
{
    if (finished) self.keyboardAvoidingState.keyboardAnimationInProgress = NO;
}
...
- (void) TPKeyboardAvoiding_keyboardWillShow: (NSNotification *) notification
{
    ...
    if (CGRectIsEmpty(keyboardRect)) return;

    TPKeyboardAvoidingState *state = self.keyboardAvoidingState;

    if (state.ignoringNotifications) return;
    ...
    if ([self isKindOfClass: [TPKeyboardAvoidingScrollView class]])
    {
        ...
        if (CGSizeEqualToSize(self.contentSize, CGSizeZero)) self.contentSize = [self TPKeyboardAvoiding_calculatedContentSizeFromSubviewFrames];
    }
    ...
    ^{
        ...
        if (firstResponder)
        {
            ...
            [self setContentOffset: CGPointMake(self.contentOffset.x, [self TPKeyboardAvoiding_idealOffsetForView: firstResponder withViewingAreaHeight: viewableHeight]) animated: NO];
        }
...
{
    CGRect keyboardRect = [self convertRect: [[[notification userInfo] objectForKey: _UIKeyboardFrameEndUserInfoKey] CGRectValue] fromView: nil];
    if (CGRectIsEmpty(keyboardRect) && !self.keyboardAvoidingState.keyboardAnimationInProgress) return;
    ...
    if ([self isKindOfClass: [TPKeyboardAvoidingScrollView class]]) self.contentSize = state.priorContentSize;
...
- (void) TPKeyboardAvoiding_scrollToActiveTextField
{
    ...
    CGPoint idealOffset = CGPointMake(self.contentOffset.x, [self TPKeyboardAvoiding_idealOffsetForView: firstResponder withViewingAreaHeight: visibleSpace]);
...
- (void) TPKeyboardAvoiding_findNextInputViewAfterView: (UIView *) priorView beneathView: (UIView *) view bestCandidate: (UIView **) bestCandidate
{
    ...
    for (UIView *childView in view.subviews)
    {
        if ([self TPKeyboardAvoiding_viewIsValidKeyViewCandidate: childView])
        {
            ...
            if (childView != priorView && ((fabs(CGRectGetMinY(frame) - CGRectGetMinY(priorFrame)) < FLT_EPSILON && CGRectGetMinX(frame) > CGRectGetMinX(priorFrame)) || CGRectGetMinY(frame) > CGRectGetMinY(priorFrame)) && (!*bestCandidate || heuristic > bestCandidateHeuristic) )
            {
                *bestCandidate = childView;
                bestCandidateHeuristic = heuristic;
            }
        }
        else [self TPKeyboardAvoiding_findNextInputViewAfterView: priorView beneathView: childView bestCandidate: bestCandidate];
    }
}

- (CGFloat) TPKeyboardAvoiding_nextInputViewHeuristicForViewFrame: (CGRect) frame
{
    return (-frame.origin.y * 1000.0f) + (-frame.origin.x);
}
...
- (BOOL) TPKeyboardAvoiding_viewHiddenOrUserInteractionNotEnabled: (UIView *) view
{
    while (view)
    {
        if (view.hidden || !view.userInteractionEnabled) return YES;
        view = view.superview;
    }
...
- (BOOL) TPKeyboardAvoiding_viewIsValidKeyViewCandidate: (UIView *) view
{
    ...
    if ([view isKindOfClass: [UITextField class]] && ((UITextField *) view).enabled) return YES;
    if ([view isKindOfClass: [UITextView class]] && ((UITextView *) view).isEditable) return YES;
...
- (CGFloat) TPKeyboardAvoiding_idealOffsetForView: (UIView *) view withViewingAreaHeight: (CGFloat) viewAreaHeight
{
    ...
    __block CGFloat padding = 0.0f;

    void(^centerViewInViewableArea)()  =
    ^{
        padding = (viewAreaHeight - subviewRect.size.height) / 2;
        if (padding < kMinimumScrollOffsetPadding) padding = kMinimumScrollOffsetPadding;
    ...
    if ([view conformsToProtocol: @protocol(UITextInput)])
    {
        ...
        else centerViewInViewableArea();
    }
    else centerViewInViewableArea();
    
    CGFloat maxOffset = contentSize.height - viewAreaHeight - self.contentInset.top;
    if (offset > maxOffset) offset = maxOffset;
    if (offset < -self.contentInset.top) offset = -self.contentInset.top;
...
{
    if ([view isKindOfClass: [UITextField class]] && ((UITextField *) view).returnKeyType == UIReturnKeyDefault && (![(UITextField *) view delegate] || [(UITextField *) view delegate] == (id <UITextFieldDelegate>) self))
    {
        ...
        ((UITextField *) view).returnKeyType = (otherView ? UIReturnKeyNext : UIReturnKeyDone);
    }
}
