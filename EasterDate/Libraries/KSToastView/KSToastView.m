...
//Parametri costanti per la costruzione della notifica.
...
#define ks_TOAST_VIEW_TEXT_FONT_SIZE ([Macros isIPad] ? 20.0f : 15.5f)

//Parametri variabili per la costruzione della notifica.
static UIColor *_backgroundColor = nil;
static UIColor *_textColor = nil;
static UIFont *_textFont = nil;
static CGFloat _cornerRadius;
...
//Metodo gestore della visualizzazione della notifica.
+ (void) ks_showToast: (id) toast duration: (NSTimeInterval) duration delay: (NSTimeInterval) delay completion: (KSToastBlock) completion
{
    ...
    ^{
		...
		CGFloat toastTextHeight = [toastText sizeWithAttributes: @{NSFontAttributeName: [self _textFont],}].height + 0.5f;

		if (UIEdgeInsetsEqualToEdgeInsets(_textInsets, UIEdgeInsetsZero)) _textInsets = UIEdgeInsetsMake(toastTextHeight / 3.0f, toastTextHeight / 3.0f, toastTextHeight / 3.0f, toastTextHeight / 3.0f);

        _cornerRadius = ([Macros isIPad] ? 10.0f : 8.0f);
        toastView.layer.cornerRadius = ((_cornerRadius <= 0.0f || _cornerRadius > toastTextHeight / 2.0f) ? (toastTextHeight + _textInsets.top + _textInsets.bottom) / 2.0f : _cornerRadius);
		...
		if (toastViewWidth > _maxWidth) toastViewWidth = _maxWidth;
        if (_maxLines > 0) toastViewHeight = toastTextHeight * _maxLines + _textInsets.top + _textInsets.bottom;
		if (toastViewHeight > _maxHeight) toastViewHeight = _maxHeight;

		NSDictionary *views = NSDictionaryOfVariableBindings(toastLabel, toastView);
		[toastView addSubview: toastLabel];
        [keyWindow addSubview: toastView];

		[toastView addConstraints: [NSLayoutConstraint constraintsWithVisualFormat: [NSString stringWithFormat: @"H:|-(%@)-[toastLabel]-(%@)-|", @(_textInsets.left), @(_textInsets.right)] options: 0 metrics: nil views: views]];
		[toastView addConstraints: [NSLayoutConstraint constraintsWithVisualFormat: [NSString stringWithFormat: @"V:|-(%@)-[toastLabel]-(%@)-|", @(_textInsets.top), @(_textInsets.bottom)] options: 0 metrics: nil views: views]];

		[keyWindow addConstraints: [NSLayoutConstraint constraintsWithVisualFormat: [NSString stringWithFormat: @"H:[toastView(%@)]", @(toastViewWidth)] options: 0 metrics: nil views: views]];
		[keyWindow addConstraints: [NSLayoutConstraint constraintsWithVisualFormat: [NSString stringWithFormat: @"V:|-(>=%@)-[toastView(<=%@)]-(%@)-|", @(_offsetTop), @(toastViewHeight), @(_offsetBottom)] options: 0 metrics: nil views: views]];
		[keyWindow addConstraint: [NSLayoutConstraint constraintWithItem: toastView attribute: NSLayoutAttributeCenterX relatedBy: NSLayoutRelationEqual toItem: keyWindow attribute: NSLayoutAttributeCenterX multiplier: 1.0f constant: 0.0f]];
...
//Metodo getter per il font del testo visualizzato.
+ (UIFont *) _textFont
{
    if (_textFont == nil) _textFont = [UIFont fontWithName: @"Roboto-Regular" size: ks_TOAST_VIEW_TEXT_FONT_SIZE];
	return _textFont;
}

//Metodo getter per il colore del testo visualizzato.
+ (UIColor *) _textColor
{
	if (_textColor == nil) _textColor = [UIColor whiteColor];
	return _textColor;
}

//Metodo getter per il colore di sfondo.
+ (UIColor *) _backgroundColor
{
	if (_backgroundColor == nil) _backgroundColor = [[UIColor blackColor] colorWithAlphaComponent: 0.8f];
	return _backgroundColor;
}

//Metodo getter per l'altezza massima consentita.
+ (CGFloat) _maxHeight
{
	if (_maxHeight <= 0) _maxHeight = [self _portraitScreenHeight] - (_offsetBottom + ks_TOAST_VIEW_OFFSET_TOP);
	return _maxHeight;
}

//Metodo getter per la larghezza massima consentita.
+ (CGFloat) _maxWidth
{
	if (_maxWidth <= 0) _maxWidth = [self _portraitScreenWidth] - ( ks_TOAST_VIEW_OFFSET_LEFT_RIGHT + ks_TOAST_VIEW_OFFSET_LEFT_RIGHT);
	return _maxWidth;
}

//Metodo getter per la larghezza dello schermo in orientamento Portrait.
+ (CGFloat) _portraitScreenWidth
{
	return UIInterfaceOrientationIsPortrait([UIApplication sharedApplication].statusBarOrientation) ? [Macros screenWidth] : [Macros screenHeight];
}

//Metodo getter per l'altezza dello schermo in orientamento Portrait.
+ (CGFloat) _portraitScreenHeight
{
	return UIInterfaceOrientationIsPortrait([UIApplication sharedApplication].statusBarOrientation) ? [Macros screenHeight] : [Macros screenWidth];
}
